...
function clickBtn() {
	...
	$("#insertExcel").click(
			function() {
				var file = $("#articleImageFile").val();
				var fileName = file.substring(file.lastIndexOf('.'),
						file.length);
				if (fileName == '') {
					showMessage("上传文件不能为空！", "error", "1000")
				} else {
					if(fileName == ".xls"){
						var formData = new FormData();
						formData.append("file", document
								.getElementById("articleImageFile").files[0]);
						formData.append("name", file);// 可以使用formData.append()传递多个参数
						$.ajax({
							...
							success : function(data) {
								showMessage(data, "success", "5000");
								initPage("2");
							}
						});
					}else{
						showMessage(fileName+"文件格式不支持,请上传Excel文件！", "error", "2000")
					}
	...
	$("#btn_dc")
			.click(
					function() {
						// 批量导出id
						var rows = $("#tb_departments").bootstrapTable(
								"getSelections");
						var queryId = "";
						for (var i = 0; i < rows.length; i++) {
							if (i == 0) {
						...
						window.location.href = "/ExcelController/downloadAllClassmate?queryId="
								+ queryId
								+ "&name="
								+ name
								+ "&address="
